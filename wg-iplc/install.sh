...
DEBIAN_FRONTEND=noninteractive apt install -y \
    wireguard wireguard-tools iptables iproute2 curl
...
if [ -z "${IPLC_IF}" ]; then
    echo "错误：没有找到 IPLC IP ${IPLC_IP} 所在网卡"
    ip -br -4 addr
    exit 1
fi
...
if ss -H -lunp 2>/dev/null | grep -Eq ":${WG_PORT}\b" && \
   ! systemctl is-active --quiet "wg-quick@${WG_IF}" 2>/dev/null; then
    echo "错误：UDP ${WG_PORT} 已被其他程序占用"
    exit 1
fi

echo "[3/8] 准备 WireGuard 密钥..."
mkdir -p "${WG_DIR}"
chmod 700 "${WG_DIR}"
umask 077
...
echo "======================================================"
echo "入口：${IPLC_MOBILE_ENTRY}:${WG_PORT}/UDP"
echo "服务端：${WG_SERVER_IP}/24"
echo "客户端：${WG_CLIENT_IP}/24"
echo "出口：${JP_IP}"
echo
